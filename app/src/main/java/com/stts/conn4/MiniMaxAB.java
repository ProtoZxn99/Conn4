...
public class MiniMaxAB {
    ...
    public MiniMaxAB(int ai, int ply) {
        this.ai = ai;
        this.ply = ply;
    }
    
    public int findmove(Board b){
        Board best = AI(b, Global.lose_ratio, Global.win_ratio);
        best.printboard();
        return best.move;
    }
    
    public Board AI(Board N, int ap, int bp){
        int alfa = ap;
        int beta = bp;
        if (N.depth>=ply||isTerminal(N)||isFull(N)) {
            ...
            if (N.depth % 2 == 0) {
                for(int i = 0; i<b.size(); i++){
                    b.set(i, AI(b.get(i),alfa, beta));
                    if(b.get(i).value>alfa){
                        alfa = b.get(i).value;
                        if(alfa>=beta){
                            return b.get(i);
                        }
